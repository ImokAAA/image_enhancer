...
class ML():
    ...
    def __compare_images(self, target, ref):
        scores = []
    ...
    def get_image_with_path(test_image_path):
        with open(test_image_path, 'rb') as img_file:
            file_bytes = np.asarray(bytearray(img_file.read()), dtype=np.uint8)
            img = cv2.imdecode(file_bytes, 1)
        return img

    def __upScaleEDSR(self, image):
        # Create an SR object
    ...
    def save(typ, img):
        ...
        fn =os.path.join(os.path.dirname(__file__), typ + '.png') # Couldnt find a way to get the correct system dir path so dont change the name of the file this is a mess. I repeat This is a mess .
        cv2.imwrite(fn,img)
